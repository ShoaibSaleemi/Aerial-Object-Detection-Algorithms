...
Usage:
    python "tools/tune_wbf_6.py"
    python "tools/tune_wbf_6.py" --trials 50 --seed 42
"""
...
WBF_PATH = PROJECT_ROOT / "scripts" / "algorithms" / "weighted_boxes_fusion_6.py"
if not WBF_PATH.exists():
    raise FileNotFoundError(f"WBF script not found: {WBF_PATH}")

_spec = importlib.util.spec_from_file_location("weighted_boxes_fusion_6", WBF_PATH)
...
BEST_JSON = PROJECT_ROOT / "runs" / "detect" / "tune_wbf" / "best_params_bayesian_6.json"
