...
Video inference: Weighted Boxes Fusion  →  Kalman + LSTM multi-object tracking
                 + STL / RTM requirement-based class overrides.
...
Pipeline per frame:
  ...
  6. STL / RTM checker overrides the class label when a physical rule fires:
       REQ-01  flash 40–100 Hz + red crop        → unknown
       REQ-02  aspect-ratio const + velocity ≈ 0 → unknown
       REQ-03  YOLO confidence < threshold        → bird
       REQ-04  shape deformation > ε              → bird
...
class STLChecker:
    ...
    per frame. Each method returns True when the STL *antecedent* is satisfied
    (meaning the consequent class override should be applied).

    REQ-01  [(40≤f≤100 Hz) ∧ (colour=red)]  → class=unknown
    REQ-02  [(AR≈const) ∧ (v≈0)]            → class=unknown
    REQ-03  [(light=off)]                    → class=bird
    REQ-04  [(Δshape>ε)]                     → class=bird
    """
    ...
    @staticmethod
    def req03_no_light(conf: float) -> bool:
        """REQ-03: YOLO confidence below threshold → no light source → bird."""
        return conf < REQ03_LIGHT_CONF_MAX
    ...
    def check(
        ...
        """
        Evaluate all four requirements independently, then apply the
        priority-ordered class override.

        Returns
        -------
        fires    : dict[str, bool]  — which antecedents fired this frame
...
def main():
    ...
    output_path = output_dir / f"{video_path.stem}_wbf_tracking_stl.mp4"
    ...
    while True:
        ...
        # Stage 3 — STL / RTM class overrides
        id_to_trk = {trk.id: trk for trk, _, _c, __ in tracker.tracks}
        stl_overridden: list = []
        for box_xyxy, track_id, cls, conf, lstm_pred in track_results:
            trk = id_to_trk.get(track_id)
            if trk is not None:
                _fires, override, _fired_id = STLChecker.check(
                    trk, conf, frame, width, height, fps
                )
                if override is not None:
                    cls = override
            stl_overridden.append((box_xyxy, track_id, cls, conf, lstm_pred))
        track_results = stl_overridden
    ...
    if has_gt and exist1_frames > 0:
        ...
        fig.tight_layout()
        plot_path = output_dir / f"{video_path.stem}_wbf_stl_eval.png"
        ...
        fig2.suptitle("WBF + STL tracking  —  per-frame metrics", fontsize=15, fontweight="bold")
        ...
        fig2.tight_layout()
        perframe_plot_path = output_dir / f"{video_path.stem}_wbf_stl_perframe.png"
        fig2.savefig(str(perframe_plot_path), dpi=300)
        plt.close(fig2)
        print(f"  Per-frame plot → {perframe_plot_path}")

        npz_path = output_dir / f"{video_path.stem}_wbf_stl_perframe.npz"
        np.savez(
            str(npz_path),
        ...
        csv_path = output_dir / f"{video_path.stem}_wbf_stl_eval.csv"
